...
Required skills in .claude/skills/:
  ...
  explore/SKILL.md               — fallback: built-in Socratic questions (degraded)
  goap-research/SKILL.md         — fallback: direct web_search (degraded)
  problem-solver-enhanced/SKILL.md — fallback: First Principles + SCQA only (degraded)
...
If any critical skill is missing → stop and inform user to re-run toolkit generator.
If any optional skill is missing → warn user about degraded quality, continue.
...
6. Ensure Architecture.md is consistent with project root Architecture (NestJS + React + PostgreSQL + Redis + Docker Compose)
7. Git commit: `docs(feature): SPARC planning for <feature-name>`

**Checkpoint:** Show SPARC summary, ask to proceed to validation.
...
Use swarm of 5 agents to validate:
...
| validator-architecture | Architecture.md | Consistency with project Architecture (NestJS, Prisma, BullMQ, React) |
...
1. Run all 5 validators in parallel (Task tool)
...
**Checkpoint:** Show validation results, ask to proceed to implementation.
...
- Backend: follow NestJS module pattern (module, controller, service, DTOs)
- Frontend: follow Next.js App Router pattern with Zustand + React Query
- Workers: follow BullMQ processor pattern with proper error handling
- Database: create Prisma migrations for schema changes

**Checkpoint:** Show implementation summary, ask to proceed to review.
...
| architecture | Integration | Consistency with project architecture (NestJS modules, Prisma, BullMQ) |
| security | Security surface | Input validation, credential encryption (AES-256), JWT handling, 152-FZ compliance |
| performance | Hot paths | Email sending throughput, warmup batch processing, AI generation latency |
| testing | Test coverage | Edge cases, missing tests, Jest + Supertest coverage |
