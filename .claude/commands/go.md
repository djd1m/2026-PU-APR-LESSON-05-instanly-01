...
| Has external API integration (SMTP, IMAP, OpenAI) | +2 |
| Requires new database entities (Prisma schema change) | +2 |
| Has cross-module dependencies (e.g., warmup + email + accounts) | +3 |
...
2. Where possible, spawn concurrent tasks:
   ...
   - Parallel frontend (React) + backend (NestJS) if independent
...
- Spawn concurrent agents when feature touches multiple modules (e.g., auth + accounts + frontend)
