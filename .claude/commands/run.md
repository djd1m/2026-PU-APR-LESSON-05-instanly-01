...
1. Check if project is already bootstrapped:
   - IF `docker-compose.yml` exists AND `src/` directory exists AND `prisma/schema.prisma` exists → skip to Step 2
   - ELSE → run `/start`
2. Verify bootstrap succeeded:
   - Project structure exists (src/auth, src/campaigns, src/email, etc.)
...
LOOP:
    ...
    4. Verify implementation:
       - Run project tests: `npm run test` to ensure no regressions
